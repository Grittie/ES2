 ...
 *   Answer: the const modifier is used to declare a variable as a constant. You can't change the value of a constant variable. You use it when you want to declare a variable that you don't want to change.
 *
 * 6c: Does the ARM processor use the stack or registers for local variables?
 *  Answer: the ARM processor uses the stack for local variables.
 *
 * 6d: What is a dangling pointer?
 *  Answer: a dangling pointer is a pointer that points to a memory location that has been deleted.
 *
